...
import (
	...
	"github.com/Z00mZE/url-shortner/ent/service/migrate"
...
func Open(driverName, dataSourceName string, options []Option, dbOption []DBOption) (*Client, error) {
	switch driverName {
	case dialect.MySQL, PgPgx, dialect.Postgres, dialect.SQLite:
		...
		return NewClient(append(options, Driver(sql.OpenDB(driverMap[driverName], db)))...), nil
	default:
...
func (c *Client) Tx(ctx context.Context) (*Tx, error) {
	if _, ok := c.driver.(*txDriver); ok {
		return nil, fmt.Errorf("service: cannot start a transaction within a transaction")
	}
	tx, err := newTx(ctx, c.driver)
	if err != nil {
		return nil, fmt.Errorf("service: starting a transaction: %w", err)
	}
	cfg := c.config
	cfg.driver = tx
	return &Tx{
